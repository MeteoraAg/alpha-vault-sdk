...
import { AlphaVault, DYNAMIC_AMM_PROGRAM_ID, PoolType } from "../alpha-vault";
...
async function createCustomizableDynamicPoolWithPermissionlessVault(
  ...
  const createAlphaVaultTx =
    await AlphaVault.createCustomizableFcfsPermissionlessVault(
      connection,
      {
 ...
 */
createCustomizableDynamicPoolWithPermissionlessVault(connection, payer)
  .then(() => {
    console.log("Done");
  })
  .catch(console.error);
