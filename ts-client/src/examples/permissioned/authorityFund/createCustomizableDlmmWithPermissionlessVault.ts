...
async function createCustomizableDlmmWithPermissionlessVault(
  ...
  const createAlphaVaultTx =
    await AlphaVault.createCustomizableProrataPermissionedVaultWithAuthorityFund(
      connection,
      {
...
/**
 * This example shows how to create dlmm with permissionless vault
 */
createCustomizableDlmmWithPermissionlessVault(connection, payer)
  .then(() => {
    console.log("Done");
  })
  .catch(console.error);
